...
type Props = {
    ...
    lang?: Locale; // ✅ add if you have it
};
            ...
            <div className="px-6 mb-6" ref={wrapperRef}>
                <div className="relative w-full">
                    <input
                        ...
                        className="w-full bg-white border border-app-card  rounded-full py-2 pr-10 pl-8 text-start focus:outline-none focus:border-app-gold shadow-sm font-active text-xs"
                    />
                    ...
                    {showResults && (
                        <div className="absolute z-50 mt-2 w-full bg-white rounded-[1.5rem] shadow-lg border border-app-card/30 overflow-hidden">
                            {/* states */}
                            {!shouldSearch && (
                                <div className="p-4 text-sm text-app-textSec text-start">
                                    {t.typeToSearch}
                                </div>
                            )}
                                ...
                                <div className="max-h-[320px] overflow-y-auto no-scrollbar">
                                    {results.map((s: any) => (
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 p-3.5 hover:bg-app-bg active:bg-app-bg transition text-start"
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="text-[11px] text-app-textSec truncate">
                                                    {s.category?.name ? `${t.section}: ${s.category.name}` : s.description}
                                                </div>
                                            </div>

                                            <div className="flex flex-col items-end gap-1">
                                                <span className="text-xs font-semibold text-app-gold">
                                                    {s.current_price} {t.currency}
                                                </span>
                                                {s.has_discount && (
